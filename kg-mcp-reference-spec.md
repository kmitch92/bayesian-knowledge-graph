...
**Version:** 0.8.0
**Date:** 2026-09-06 (baseline v0.1.0 frozen 2026-08-04)
...
**A concrete extractor exists.** `AnthropicExtractor` — one call to Anthropic's Messages API per chunk, forcing a single tool (`record_claims`) — is the first non-fixture implementation of this section's port, and it serves §5.11's transcript chunks too: same port, same adapter, whichever channel the chunk came from. Its system prompt carries the tier rubric above as prose, because no `input_schema` enum can encode *which* rung a claim earns — only argued cases can. The claim-with-quote gate is read exactly as written: `chunkText.includes(quote)`, blank quotes refused, and nothing else — no trim, no case fold, no smart-quote or whitespace normalization anywhere on the path, so a model that tidies its own citation loses the claim to the extraction-rejection log rather than the graph. **Extraction quality remains unmeasured.** Every test drives this adapter against a hand-built, injected response; no chunk of any document has yet been sent to the live API. The rejection rate — how often a model can quote itself verbatim — is the first thing a live run would report, and none has run (§14).
