...
**Version:** 0.9.3
**Date:** 2026-09-14 (baseline v0.1.0 frozen 2026-08-04)
**Status:** design-complete; implementation in progress — the write path (`kgmem init`, `kgmem ingest`, `kgmem reflect`) is built, the read path (`kgmem mcp`) is not
...
**The prompt carries a cache breakpoint now, and the breakpoint is inert (E9b).** `system` moved from a bare string to a one-block content list with `cache_control: { type: 'ephemeral' }` on that block. Anthropic builds its cached prefix in the order `tools`, `system`, `messages`, so the one marker on the system block already covers the tool schema ahead of it; a second marker there would spend one of the four breakpoints Anthropic allows on a prefix the first already covers byte for byte. The chunk and the drain's context stay in `messages`, after the breakpoint, so the cached bytes are two module constants, identical on every chunk of every document. `usage` is now Zod-parsed and reaches a caller through an `onUsage` constructor option rather than through `extract`'s return — widening this section's `(chunk) → claims` port would carry Anthropic's billing fields into the two scripted extractors that satisfy it and have no bill of their own — with fields renamed to camelCase and an omitted figure left absent rather than defaulted to `0`, because an absent `cache_read_input_tokens` and a reported zero are different facts, and conflating them is exactly how E7d and E8d's sixty calls between them went by at zero unnoticed. The marker earns nothing yet: `claude-haiku-4-5`'s minimum cacheable prefix is 4,096 tokens, the prompt and tool schema together measure about 2,798, and Anthropic's documented behaviour below the floor is to serve the request as if unmarked and report no error — so this buys structure and the observability to tell a hit from a miss, not a cost saving, until a live call reports `cacheReadInputTokens` above zero (§14.20).
...
15. **Resolved (v0.9.3): `kgmem reflect` exits 4 when every model call it made failed** (§5.10, §9): the drain still catches a thrown `Extractor` and hands the job back or parks it, never rethrowing, but its outcome now says whether the model was asked and whether the attempt failed, and `reflect` counts both. A run exits 4 (`Failed`) when at least one chunk's model call failed and no chunk's model call succeeded — a full outage, a bad key, or every answer truncated. A run where some calls failed exits 0. A job parked before the model was asked (its document or chunk is gone) is neither a success nor a failure. The tally line always carries the failure count — `reflected over N chunks: A members admitted, R rejected, F failed (X will retry, Y parked)`, or `0 failed` — and an exit-4 run adds one line quoting the last error the model raised, so an operator can tell a bad key from an outage. A model answer the gate refuses in full, or an empty answer, is a success: asking again asks the same question.
...
20. **The cache breakpoint is inert below the model's cacheable-prefix floor** (§5.10, E9b): `claude-haiku-4-5`'s minimum cacheable prefix is 4,096 tokens; `EXTRACTION_PROMPT` plus the tool schema together measure about 2,798, roughly 1,300 short, and Anthropic's documented behaviour below the floor is to serve the request exactly as if unmarked, with no error anywhere — so the marker E9b put on `system` is correct structure with no live evidence yet that it does anything, and none is possible until a call reports `cacheReadInputTokens` above zero. Three ways to close the gap sit open, none ruled on here: pad the prompt past the floor, at the cost of growing this system's one reviewable artefact for a purpose other than extraction quality; batch several chunks into one call so the fixed prefix is billed fewer times rather than more cheaply per call; or move to a model with a lower floor, blocked today by item 12's finding that model choice is not reachable from `.kgmem/config.json` at all. Leaving the marker as it is and paying the full prefix on every call remains the default until one of those is chosen.
...
**v0.9.2 (2026-09-13) — the extraction adapter requests caching and surfaces what it was billed, both inert without a live call (E9b).** `system` moves from a bare string to a one-block content list carrying `cache_control: { type: 'ephemeral' }` on that block; Anthropic's cached-prefix build order (`tools`, `system`, `messages`) makes the one marker sufficient to cover the tool schema ahead of it, so a second marker there would spend one of the API's four allowed breakpoints naming a prefix the first already covers. The chunk and the drain's context stay in `messages`, after the breakpoint, so the cached bytes are two module constants, identical across every chunk of every document. `usage` is now Zod-parsed and reaches a caller through an `onUsage` constructor option — widening §5.10's `(chunk) → claims` port would carry Anthropic's billing fields into two scripted extractors with no bill of their own — with fields renamed to camelCase and an omitted figure left absent rather than defaulted to `0`, since an absent `cache_read_input_tokens` and a reported zero are the two facts E7d and E8d's sixty calls between them conflated at zero, unread. §14 gains one item (20): the marker is inert today, because `claude-haiku-4-5`'s 4,096-token cacheable-prefix floor sits about 1,300 tokens above the measured ~2,798-token prompt-plus-schema, and Anthropic serves a sub-floor request unmarked with no error — so this entry buys structure and the observability to tell a hit from a miss, not a cost saving yet.

**v0.9.3 (2026-09-14) — a workspace can be created, and an extraction outage no longer exits 0.** `kgmem init [path]` creates `.kgmem/` with a migrated empty store and a `{"models":{}}` configuration in the given directory (default: the working directory) and does nothing else: it never reads the directory's content, which enters only through `kgmem ingest`. Run inside a directory that already belongs to a workspace, it reports that workspace and creates nothing; run over a half-made `.kgmem/`, it creates only the missing store or configuration and leaves existing files untouched. A missing path, a path that is a file, or a file named `.kgmem` in the way exits 1 (`Usage`). Separately, §14.15 is resolved: `kgmem reflect` exits 4 when at least one model call failed and none succeeded, and its tally line always carries a failure count. Neither change alters §3–§13's model.
