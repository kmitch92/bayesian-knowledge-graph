# WIP — after E11: the read path serves an agent; the live trial is the open item

**Status:** E7d–E10 as below, plus E11 (2026-09-17): `kgmem mcp` serves `query` and `observe` over stdio. Built and smoke-tested against the live workspace; the user-run live trial inside Claude Code has not happened yet.
**Companions:** reference spec v0.9.3 · v1 implementation plan 1.9

---

## E11 — `kgmem mcp`, and the live trial that has not run yet

- **What ships.** `kgmem mcp` (`/home/kiel/dev/bayesian-knowledge-graph/src/adapters/cli/mcp.ts`) is a stdio MCP server holding one workspace store for the life of the process, with two tools. `query` resolves an anchor from the task text (entity id, then the mention index, then 1–3-word runs of the task, then gloss similarity), gathers claims at the anchor and its containment ancestors, falls back to claim-embedding search when nothing anchors, ranks by band × posterior mean × status penalty, packs to `budgetTokens` at 4 characters per token, rides contradicting rivals along past the budget, and records a taint row per served claim. `observe` writes one claim through the ingest port under the session episode, channel `mcp-observe`. One process is one episode, id `mcp:<ISO start time>`.
- **Decisions made while building, none of them in the spec.** Status penalties active 1.0 / disputed 0.75 / provisional 0.5, with deprecated and archived never served; ancestor decay 0.8 per level; no freshness factor; `hint` accepted and ignored; a missing containment level reported as `unplaced`; `observe`'s `about` is required with at least one name (the write path needs ≥1 mention, so an optional field could only fail later); a repeat of the same text in the same episode lands a second claim row and answers `duplicate: true` — the row lands, the posterior does not move.
- **Bookkeeping claims are not served.** The existence and naming rows the referent index is a view over carry the `kgmem-spine:` footer written by `/home/kiel/dev/bayesian-knowledge-graph/src/referents/spine.ts`. They are 243 of the 343 claims in `/home/kiel/kgmem-live-e8d/.kgmem/graph.db` and outranked real content, so `gather` now drops any claim whose text decodes as a spine payload (user ruling, 2026-09-17).
- **Smoke test, read-only, already run.** The built server at `/home/kiel/dev/bayesian-knowledge-graph/dist/kgmem.js` was driven by hand-written JSON-RPC over stdin from `/home/kiel/kgmem-live-e8d`: it initializes, lists both tools, and answers "what does the spec say about grouping claims" with two content claims anchored on the entity `spec` (level `unplaced`). A question with no word overlap with the graph returned nothing — the 0.70 cosine floor was never reached, which may be honest silence rather than a defect. `observe` was deliberately not exercised against the live graph, because it writes.
- **The live trial, still to run.** Claude Code must be restarted for a newly registered server to appear, which is why this is written down. From `/home/kiel/kgmem-live-e8d`: `claude mcp add kgmem -- node /home/kiel/dev/bayesian-knowledge-graph/dist/kgmem.js mcp`, restart, then in a session started in that directory ask a question the graph knows, record something with `observe`, and ask again so the same session serves back what it just wrote. Both tools use the local embedding model only; no paid calls. `observe` adds real rows to `/home/kiel/kgmem-live-e8d/.kgmem/graph.db` — copy it first if that matters. Worth recording from the trial: whether the anchor picked is the one a reader would pick, whether 2000 tokens of claims is useful or noise, whether all-provisional claims with width ~0.92 read as usable, and what the agent does with `duplicate: true`.
- **Gated or absent.** `query` with `modes: ["traverse"]` is refused with NOT_IMPLEMENTED; `contradict` and `drill_down` have no write path and are not registered; taint is recorded but nothing yet stops a restatement from counting as evidence (that needs claim matching, P3).
